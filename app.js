...
function hasPaymentEvidence(m){
  if(!m) return false;
  const ledger=readPaymentLedger();
  return (Array.isArray(m.payments)&&m.payments.length>0)||(Array.isArray(ledger[m.id])&&ledger[m.id].length>0);
}
function normalizeMilestones(candidate){
  ...
  const out=canonical.map(base=>{
    ...
    if(safeNumber(merged.balance)<=0 && safeNumber(base.balance)>0 && !hasPaymentEvidence(old)) merged.balance=base.balance;
    if(safeNumber(merged.starting)<=0) merged.starting=base.starting;
    return merged;
  });
  const debt=out.filter(m=>m.kind==='debt');
  if(debt.reduce((s,m)=>s+safeNumber(m.balance),0)<=0.01 && !debt.some(hasPaymentEvidence)){
