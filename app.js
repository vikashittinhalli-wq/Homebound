...
function greeting() {
  ...
  const portrait = document.getElementById('familyPortrait');
  if (portrait) {
    portrait.setAttribute('aria-label', `${greetingText}. You and your wife together, looking towards home.`);
  }
}
...
document.addEventListener('submit', event => {
  if (event.target.id === 'paymentForm') {
    ...
    const amount = parseCurrency(data.get('amount'));
    ...
    if (persisted) {
      ...
      toast(`✅ Saved ${money2(amount)} • New balance ${money2(m.balance)}`);
    } else {
      toast('⚠️ The update could not be saved. Please try again.');
    }
  }
