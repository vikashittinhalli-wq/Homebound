...
// Last verified figures from the working Journey build. These are used only
// when a previous broken migration replaced every debt balance with zero.
...
function hasAnyPaymentEvidenceForAccount(id) {
  const ledger = readPaymentLedger();
  if (Array.isArray(ledger[id]) && ledger[id].length) return true;
  try {
    const raw = JSON.parse(localStorage.getItem(STORAGE_KEY) || 'null');
    const m = Array.isArray(raw?.milestones) ? raw.milestones.find(x => x?.id === id) : null;
    return Array.isArray(m?.payments) && m.payments.length > 0;
  } catch { return false; }
}

function repairLegacyZeros(legacy) {
  const result = { ...LEGACY_DEFAULTS, ...(legacy || {}) };
  const debtKeys = ['cc1','cc2','zipPlus','zipMoney','car'];
  const total = debtKeys.reduce((sum, key) => sum + Math.max(0, safeNumber(result[key])), 0);
  const hasEvidence = debtKeys.some(key => hasAnyPaymentEvidenceForAccount(key === 'zipMoney' ? 'zip-money' : key === 'zipPlus' ? 'zip-plus' : key));

  // A real paid-off journey would have payment evidence. An all-zero state with
  // no evidence is the corruption produced by the failed migration.
  if (total <= 0.01 && !hasEvidence) {
    debtKeys.forEach(key => { result[key] = VERIFIED_RECOVERY[key]; });
    result.updatedAt = new Date().toISOString();
    result.recoveredBy = 'homebound-v3-zero-repair';
    try { localStorage.setItem(LEGACY_KEY, JSON.stringify(result)); } catch {}
  } else {
    // Repair isolated unexplained zeroes without disturbing genuinely completed debts.
    const idMap = { cc1:'cc1', cc2:'cc2', zipPlus:'zip-plus', zipMoney:'zip-money', car:'car' };
    debtKeys.forEach(key => {
      if (safeNumber(result[key]) <= 0 && !hasAnyPaymentEvidenceForAccount(idMap[key])) {
        result[key] = VERIFIED_RECOVERY[key];
      }
    });
  }
  return result;
...
function readLegacyData() {
  try {
    return repairLegacyZeros(JSON.parse(localStorage.getItem(LEGACY_KEY) || '{}'));
  } catch {
    return repairLegacyZeros({ ...LEGACY_DEFAULTS });
  }
}
...
function normalizeMilestones(candidateMilestones) {
  ...
  // If a failed build saved an entirely zero debt map, restore the last
  // verified balances. Completed debts are never changed when payment evidence exists.
  const debtIds = new Set(['zip-money','zip-plus','cc1','cc2','car']);
  const debtTotal = normalized.filter(m => debtIds.has(m.id)).reduce((sum,m) => sum + Math.max(0,safeNumber(m.balance)),0);
  const debtEvidence = normalized.filter(m => debtIds.has(m.id)).some(m => hasRealPaymentEvidence(m));
  if (debtTotal <= 0.01 && !debtEvidence) {
    const recovered = buildMilestonesFromLegacy(repairLegacyZeros({}));
    const recoveredById = new Map(recovered.map(m => [m.id,m]));
    normalized.forEach((m,i) => {
      if (debtIds.has(m.id) && recoveredById.has(m.id)) normalized[i] = { ...m, ...recoveredById.get(m.id), payments: Array.isArray(m.payments) ? m.payments : [] };
    });
  }
...
function ensureFoundationState(){
  ...
  state.dreams.board = Array.isArray(state.dreams.board) ? state.dreams.board.filter(Boolean) : [];
  // The failed migration also persisted an empty Dream Board. Always restore the
  // six foundation cards while preserving anything the user added.
  const dreamById = new Map(state.dreams.board.map(d => [d.id, d]));
  defaultDreamBoard.forEach(d => {
    if (!dreamById.has(d.id)) state.dreams.board.push(clone(d));
  });
  ...
  state.schemaVersion = SCHEMA_VERSION;
  state.lastRecovery = state.lastRecovery || 'v3-zero-repair';
  save();
}
