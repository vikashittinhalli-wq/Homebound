...
function paymentForm(id) {
  ...
  if (!m) return;
  openSheet([m.icon, m.kind === 'savings' ? `Add to ${m.publicName}` : `Pay ${m.publicName}`, 'Record the amount and update your real balance.', `<form id="paymentForm" class="form-grid" novalidate><input type="hidden" name="id" value="${m.id}"><label>Amount<input name="amount" type="number" min="0.01" step="0.01" required inputmode="decimal" autocomplete="off" placeholder="151.17"></label><label>Date<input name="date" type="date" value="${new Date().toISOString().slice(0, 10)}" required></label><label>Type<select name="type"><option>Minimum payment</option><option>Extra payment</option><option>Minimum + extra</option><option>Statement adjustment</option></select></label><button class="primary-button" type="submit">Save update</button></form>`]);
}

function commitPayment(form) {
  const data = new FormData(form);
  const milestoneId = String(data.get('id') || '');
  const m = state.milestones.find(item => item.id === milestoneId);
  const amount = parseCurrency(data.get('amount'), NaN);
  const date = String(data.get('date') || '');
  const type = String(data.get('type') || 'Payment');

  if (!m) { toast('⚠️ Milestone not found.'); return; }
  if (!Number.isFinite(amount) || amount <= 0) { toast('⚠️ Enter an amount greater than $0.'); return; }
  if (!date) { toast('⚠️ Choose a payment date.'); return; }

  const stateBefore = clone(state);
  const ledgerBefore = readPaymentLedger();
  if (!Array.isArray(m.payments)) m.payments = [];
  let interest = 0;
  ...
  const entry = {
    ...
    amount, date, type, interest, balanceAfter: newBalance, createdAt: new Date().toISOString()
  };
  ...
  const ledgerSaved = writePaymentLedger(ledger);
  syncLegacyBalance(m);
  const stateSaved = save();

  const verified = JSON.parse(localStorage.getItem(STORAGE_KEY) || 'null');
  const verifiedMilestone = verified && Array.isArray(verified.milestones)
    ? verified.milestones.find(item => item.id === m.id)
    : null;
  const verifiedEntry = verifiedMilestone && Array.isArray(verifiedMilestone.payments)
    ? verifiedMilestone.payments.find(item => item.id === entry.id)
    : null;

  if (!ledgerSaved || !stateSaved || !verifiedEntry) {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(stateBefore));
    writePaymentLedger(ledgerBefore);
    state = mergeLedgerIntoState(stateBefore);
    render();
    toast('⚠️ Save failed. Nothing was changed. Please try again.');
    return;
  }

  state = mergeLedgerIntoState(verified);
  render();
  milestoneSheet(m.id, 'payments');
  toast(`✅ Saved ${money2(amount)} • New balance ${money2(newBalance)}`);
}
