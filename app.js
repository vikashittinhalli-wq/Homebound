...
/* Working-base recovery and story normalization */
const VERIFIED_RECOVERY = { cc1:9209.08, cc2:18803, zipPlus:3846, zipMoney:2509, car:27466.39, fund:0 };
function hasConfirmedCompletion(m){
  if(!m) return false;
  const ledger=readPaymentLedger();
  const entries=[...(Array.isArray(m.payments)?m.payments:[]), ...(Array.isArray(ledger[m.id])?ledger[m.id]:[])];
  if(m.completedAt || m.status === 'completed') return true;
  const starting=safeNumber(m.starting);
  const paid=entries.reduce((sum,p)=>sum+Math.max(0,safeNumber(p.amount)),0);
  return starting > 0 && paid >= starting - 0.01;
}
function normalizeMilestones(candidate){
  const legacy=readLegacyData();
  const canonical=buildMilestonesFromLegacy(legacy);
  const existing=Array.isArray(candidate)?candidate:[];
  const byId=new Map(existing.map(m=>[m.id,m]));
  const out=canonical.map(base=>{
    const old=byId.get(base.id);
    if(!old) return clone(base);
    const merged={...clone(base),...old,id:base.id,payments:Array.isArray(old.payments)?old.payments:[]};
    if(safeNumber(merged.balance)<=0 && safeNumber(base.balance)>0 && !hasConfirmedCompletion(old)) merged.balance=base.balance;
    if(safeNumber(merged.starting)<=0) merged.starting=base.starting;
    return merged;
  });
  const debt=out.filter(m=>m.kind==='debt');
  if(debt.reduce((s,m)=>s+safeNumber(m.balance),0)<=0.01 && !debt.every(hasConfirmedCompletion)){
    const values={ 'zip-money':VERIFIED_RECOVERY.zipMoney,'zip-plus':VERIFIED_RECOVERY.zipPlus,cc1:VERIFIED_RECOVERY.cc1,cc2:VERIFIED_RECOVERY.cc2,car:VERIFIED_RECOVERY.car };
    out.forEach(m=>{ if(values[m.id]!=null){m.balance=values[m.id]; if(m.id!=='car')m.starting=Math.max(m.starting,values[m.id]);} });
    try{ localStorage.setItem(LEGACY_KEY,JSON.stringify({...readLegacyData(),...VERIFIED_RECOVERY,updatedAt:new Date().toISOString()})); }catch{}
  }
  existing.forEach(m=>{if(m&&!out.some(x=>x.id===m.id))out.push(m)});
  return mergeLedgerIntoState({milestones:out}).milestones;
}
...
function ensureFoundationState(){
  ...
  state.schemaVersion = SCHEMA_VERSION;
  state.lastRecovery = 'v4-working-base-repair';
  save();
}
