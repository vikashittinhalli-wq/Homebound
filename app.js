...
function commitPaymentDirect(milestoneId) {
  ...
  try {
    ...
    localStorage.setItem(PAYMENT_LEDGER_KEY, JSON.stringify(ledger));
    syncLegacyBalance(m);

    const persisted = JSON.parse(localStorage.getItem(STORAGE_KEY) || 'null');
    const persistedMilestone = persisted?.milestones?.find(item => item.id === m.id);
    const persistedEntry = persistedMilestone?.payments?.find(item => item.id === entry.id);
    if (!persistedEntry) throw new Error('Payment did not persist');

    state = mergeLedgerIntoState(persisted);
    render();
